...
def _learn(max_episode, epsilon, alpha, save_file):
    ...
    start_mark = 'O'
    returns={}
    temp=defaultdict(lambda: np.zeros(env.action_space.n))
    ...
    for i in tqdm(range(max_episode)):
        ...
        while not done:
            ...
            if state not in bpolicy:
                bpolicy[state]=ava_actions
            env.show_turn(False, mark)
            action = random.choice(ava_actions)
            if(state not in first_index.keys()):
                first_index[state]=j

            nstate, reward, done, info = env.step(action)
            episode_return.append((state, action, reward))
        ...
        for t in range(len(episode_return))[::-1]:
            state, action, reward = episode_return[t]
            # if(reward<=0): print(episode_return[t])
            G =  G + reward
            C[state][action] += W*1.
            temp[state][action]+=1
            Q[state][action] += 1.*(W / C[state][action]) * (G - Q[state][action])
            # if i==0:
                # print(state,action,C[state][action],G,reward,W,Q[state][action])
            # print(C[state][action])            
            # if(Q[state][action]<0): print(Q[state][action])
            A = np.zeros_like(Q[state], dtype=float)
            best_action = np.argmax(Q[state])
            # A[best_action]=1
            A[best_action] = 1-1.*epsilon* episode / float(max_episode)
            for i in range(len(A)): A[i]+=1.*epsilon*(episode / float(max_episode))/(env.action_space.n)
            # # print("kmt")
            # print(A)
            # if action !=  np.argmax(A):
            if A[action] ==  0:
                break
            W = W * 1.*A[action]/(1./bpolicy[state])
            # print(W)
        start_mark = next_mark(start_mark)
    for key in Q:
        res=[]
        for i in range(bpolicy[key]):
            res
        best_action = np.argmax(Q[key])
        set_state_value(key, Q[key][best_action])
    z=0
    for key in temp:
        z+=1
        print(key)
        print(bpolicy[key])
        st_visits[key]=bpolicy[key]
        print(Q[key])
    print(z)
    # for key in st_values:
        # print(st_values[key])     
        # n=len(episode_return)
        # for i in range(2,len(episode_return)+1):
        #     episode_return[n-i]+=episode_return[n-i+1]
        # for key in first_index.keys():
        #     if key not in returns:
        #         returns[key]=[]
        #         returns[key].append(episode_return[first_index[key]])
        #     else:
        #         returns[key].append(episode_return[first_index[key]])
        #     val2=sum(returns[key]) / len(returns[key])
        #     set_state_value(key, val2)
        # # print(returns)
        # # print(first_index[state])
        # # val2=1
        # # episode_return[first_index[state]]
        # # rotate start  
